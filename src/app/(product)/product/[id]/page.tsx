        ...
        <div className="container mx-auto px-4 py-8">
          ...
          <div className="flex flex-col md:flex-row gap-8 mb-16">
            ...
            <div className="w-full md:w-1/2">
              ...
              {/* Price */}
              {product.status!=="out-of-stock" &&<div className="mb-4">
                <div className="flex items-baseline">
                  {/* <span className="text-[#5A5A5A] mr-2">{product.price + (Math.random()*200).toFixed()}</span> */}
                  {product.mrp && <> <span className="font-semibold text-gray-600">MRP.</span><span className="line-through text-gray-500 font-semibold pl-[2px] pr-[7px]">{product.mrp}</span></>}
                  <span className="text-2xl font-medium text-emerald-400">₹{product.price}</span>
                  {product.mrp && <span className="text-amber-500 font-semibold pl-[2px] pr-[7px]">({100-Number((product.price/product.mrp *100).toFixed())}%OFF)</span>}
                </div>
                <TypographySmall>Inclusive of all taxes</TypographySmall>
              </div>}

              {product.status=="out-of-stock" && <span className="text-red-600">Out of stock</span>}
              ...
              {/* Size Selection */}
              {product.status!=="out-of-stock" && <div className="mb-6">
